...
namespace Focus.Providers.Mutagen
{
    public static class MutagenExtensions
    {
        ...
        public static string GetRealDataDirectory<TModSetter, TModGetter>(
            this IGameEnvironment<TModSetter, TModGetter> env)
        ...
        // Mutagen used to provide AsLink/AsLinkGetter extensions directly on FormKey; modern versions only provide
        // them on records, plus implicit conversions. These shims keep the many existing call sites unchanged.
        public static FormLink<TMajorGetter> AsLink<TMajorGetter>(this FormKey formKey)
            where TMajorGetter : class, IMajorRecordGetter
        {
            return new FormLink<TMajorGetter>(formKey);
        }

        public static IFormLinkGetter<TMajorGetter> AsLinkGetter<TMajorGetter>(this FormKey formKey)
            where TMajorGetter : class, IMajorRecordGetter
        {
            return new FormLink<TMajorGetter>(formKey);
        }

        // Model, icon and texture paths are exposed as typed asset links in modern Mutagen instead of plain strings.
        // This unwraps the underlying path (returning null for missing/empty links) so existing path-based code can
        // keep treating them as nullable strings.
        public static string? PathOrDefault(this IAssetLinkGetter? assetLink)
        {
            var path = assetLink?.GivenPath;
        ...
        public static TModGetter? TryGetMod<TModSetter, TModGetter>(
            this IGameEnvironment<TModSetter, TModGetter> env, string pluginName, ILogger log)
            where TModSetter : class, IContextMod<TModSetter, TModGetter>, TModGetter
            where TModGetter : class, IContextGetterMod<TModSetter, TModGetter>
        {
