        ...
        protected override void Configure(AnalysisRunner runner)
        {
            ...
            var assetPathConfig = AssetPathConfiguration.Builder()
                .For<IArmorAddonGetter>(armorAddon => armorAddon
                    .Add(AssetKind.Mesh, x => x.FirstPersonModel, x => x.File.PathOrDefault())
                    .Add(AssetKind.Mesh, x => x.WorldModel, x => x.File.PathOrDefault()))
                .For<IArmorGetter>(armor => armor
                    .Add(AssetKind.Mesh, x => x.WorldModel, x => x.Model?.File.PathOrDefault())
                    .Add(
                        AssetKind.Icon, x => x.WorldModel,
                        x => new[] { x.Icons?.SmallIconFilename.PathOrDefault(), x.Icons?.LargeIconFilename.PathOrDefault() }))
                .For<IArtObjectGetter>(artObject => artObject
                    .Add(AssetKind.Mesh, x => x.Model?.File.PathOrDefault()))
                .For<IHeadPartGetter>(headPart => headPart
                    .Add(AssetKind.Mesh, x => x.Model?.File.PathOrDefault())
                    .Add(AssetKind.Morph, x => x.Parts.Select(p => p.FileName.PathOrDefault())))
