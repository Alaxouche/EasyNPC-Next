...
namespace Focus.Apps.EasyNpc.Reports
{
    public class PostBuildReportGenerator
    {
        ...
        private bool environmentConfirmed;
        ...
        public async Task<PostBuildReport> CreateReport()
        {
            ...
            // Confirming the game environment is the expensive one-time step; a re-run reuses it.
            if (!environmentConfirmed)
            {
                await Task.Run(() => setup.Confirm());
                environmentConfirmed = true;
            }
            // Fresh index each run so a re-run picks up changes without duplicating entries.
            archiveIndex = new(archiveProvider);
            archiveSources.Clear();
