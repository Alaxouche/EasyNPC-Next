...
namespace Focus.Apps.EasyNpc.Profiles
{
    ...
    public interface INpc : INpcBasicInfo
    {
        ...
        NpcChangeResult AutoAssignFaceFromPolicy();
        NpcSetupAttributes GetPolicyRecommendation();
    ...
    public class Npc : INpc
    {
        ...
        public NpcChangeResult AutoAssignFaceFromPolicy()
        {
            // Applies the same recommendation engine as a profile reset, but scoped to the face plugin only, so a batch
            // "auto-assign" can fill in sensible faces across a filtered set without touching the Default (behavior)
            // plugin. Returns OK only when the face actually changed (Redundant if it already matched the recommendation).
            var recommendation = policy.GetSetupRecommendation(this);
            return SetFaceOption(recommendation.FacePluginName);
        }

        public NpcSetupAttributes GetPolicyRecommendation()
        {
            // Non-mutating: exposes what ApplyPolicy would choose, so callers can preview a reset before committing.
            return policy.GetSetupRecommendation(this);
        }
