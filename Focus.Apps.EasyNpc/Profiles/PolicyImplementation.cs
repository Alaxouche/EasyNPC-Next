...
namespace Focus.Apps.EasyNpc.Profiles
{
    ...
    {
        // The player character's NPC record in Skyrim.esm.
        private static readonly IRecordKey playerKey = new RecordKey("Skyrim.esm", "000007");

        private readonly Configuration.IAppSettings appSettings;
        ...
        public ProfilePolicy(Configuration.IObservableAppSettings appSettings)
        {
        ...
        {
            // The player character record is sometimes touched by other mods, but there is nothing useful for EasyNPC
            // to carry over, and patching it - in particular writing a Worn Armor to it - breaks custom and beast-race
            // player characters in game (#138). Exclude it from the profile entirely.
            if (RecordKey.Equals(npcChain.Key, playerKey))
                return false;
            // Character-generation face presets (e.g. the vanilla RaceMenu presets that mods like Better Argonian Horns
            // edit) are actor records used only by the character creation menu. They never have FaceGen files, so
            // treating them as NPCs just produces false "missing FaceGen" alerts and pointless merge work. Exclude any
            // NPC whose winning record - or any override - is flagged as a preset.
            if (npcChain.Any(r => r.Analysis.IsChargenFacePreset))
                return false;
            // Children are excluded by default: merging their appearance risks giving them an adult body if their
            // race/skin isn't handled exactly right. Opt in to include them (see the child-race handling in ApplyFace).
            if (!npcChain.Master.CanUseFaceGen || npcChain.Master.IsAudioTemplate)
                return false;
            if (npcChain.Master.IsChild && !appSettings.IncludeChildNpcs)
                return false;
