    ...
    public class SharedResourceCopyTask : BuildTask<SharedResourceCopyTask.Result>
    {
        ...
        private IEnumerable<string> GetArmorMeshes()
        {
            return patch.Mod.Armors
                .SelectMany(x => new[]
                {
                    x.WorldModel?.Male?.Model?.File.PathOrDefault(),
                    x.WorldModel?.Male?.Icons?.LargeIconFilename.PathOrDefault(),
                    x.WorldModel?.Male?.Icons?.SmallIconFilename.PathOrDefault(),
                    x.WorldModel?.Female?.Model?.File.PathOrDefault(),
                    x.WorldModel?.Female?.Icons?.LargeIconFilename.PathOrDefault(),
                    x.WorldModel?.Female?.Icons?.SmallIconFilename.PathOrDefault(),
                })
                .NotNull();
        }
        ...
        private IEnumerable<string> GetArmorAddonMeshes()
        {
            return patch.Mod.ArmorAddons
                .SelectMany(x => new[]
                {
                    x.FirstPersonModel?.Male?.File.PathOrDefault(),
                    x.FirstPersonModel?.Female?.File.PathOrDefault(),
                    x.WorldModel?.Male?.File.PathOrDefault(),
                    x.WorldModel?.Female?.File.PathOrDefault(),
                })
                .NotNull();
        }
        ...
        private IEnumerable<string> GetArtObjectMeshes()
        {
            return patch.Mod.ArtObjects
                .Select(art => art.Model?.File.PathOrDefault())
                .NotNull();
        }
        ...
        private IEnumerable<string> GetHeadPartMeshes()
        {
            return patch.Mod.HeadParts
                .Select(x => x.Model?.File.PathOrDefault())
                .NotNullOrEmpty();
        }
        ...
        private IEnumerable<string> GetHeadPartMorphs()
        {
            return patch.Mod.HeadParts
                ...
                .Select(x => x.FileName.PathOrDefault())
                .NotNullOrEmpty();
        }
    }
}
