...
{
    public class CascadingFileProvider : IFileProvider, IPhysicalFilePathProvider
    {
        ...
        {
            // Only if the highest-priority provider that has the file is itself physical; otherwise null (honors overrides).
            var matchingProvider = providers.FirstOrDefault(p => p.Exists(fileName));
            return matchingProvider is IPhysicalFilePathProvider physical ?
                physical.GetPhysicalFilePath(fileName) : null;
        }
    }
}