...
{
    public class DirectoryFileProvider : IFileProvider, IPhysicalFilePathProvider
    {
        ...
        {
            var fullPath = fs.Path.Combine(rootPath, fileName);
            return fs.File.Exists(fullPath) ? fullPath : null;
        }
    }
}