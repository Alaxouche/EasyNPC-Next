...
namespace Focus.Storage.Archives
{
    public class ArchiveBuilder
    {
        ...
        // Must be long: an int accumulator overflows past ~2 GB and silently disables the compressed-size split guard.
        private Func<FileEntry, long> estimateCompressedSize = _ => 0;
        ...
        public ArchiveBuilder AddDirectory(
            string directoryName, string pathInArchive, string searchPattern = null,
            Func<string, bool> excludePathInArchive = null)
        {
            var entries = Directory.GetFiles(directoryName, searchPattern ?? "*.*", SearchOption.AllDirectories)
                ...
                .Where(e => excludePathInArchive == null || !excludePathInArchive(e.PathInArchive));
        ...
        public ArchiveBuilder MaxCompressedSize(long maxCompressedSize, Func<FileEntry, long> estimateCompressedSize)
        {
            ...
            foreach (var entry in fileEntries)
            {
                ...
                if (group.Count > 0 &&
                    (group.Size + entry.Size > maxUncompressedSize ||
                        group.EstimatedCompressedSize + estimatedCompressedSize > maxCompressedSize))
                {
                    yield return group;
        ...
        class FileGroup
        {
            ...
            public long EstimatedCompressedSize { get; private set; }
            public string FileName { get; init; }
            public long Size { get; private set; }

            private readonly List<FileEntry> entries = new();

            public void Add(FileEntry entry, long estimatedCompressedSize)
            {
                entries.Add(entry);
                Size += entry.Size;
