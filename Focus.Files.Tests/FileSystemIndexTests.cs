...
namespace Focus.Files.Tests
{
    public class FileSystemIndexTests : IDisposable
    {
        ...
        public FileSystemIndexTests()
        {
            ...
            // MockFileSystem's watcher factory is read-only in newer System.IO.Abstractions, so wrap it to inject the
            // fake watcher used to drive change events in these tests.
            var indexFs = new WatcherOverrideFileSystem(fs, fakeFileSystemWatcherFactory);
            AddFiles(new[]
            {
                @"C:\temp\abc.txt",
            ...
            index = FileSystemIndex.Build(
                indexFs, @"C:\path\to\root", Bucketizers.TopLevelDirectory(), new[] { ".txt", ".log" });
